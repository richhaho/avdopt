              ...
              <div class="row">
                <div class="col-md-3 leftSidebar">
                  ...
                  <ul class="nav nav-tabs customtab" role="tablist">
                      ...
                      <li class="nav-item" style="width: 100%;"> <a style="background-image:url({{ asset('images/certificates.png') }}); background-repeat: no-repeat; padding: 20px 60px;" class="nav-link active" href="{{route('adoptions.certificates')}}" role="tab"><span class="hidden-sm-up"><i class="ti-email"></i></span> <span class="hidden-xs-down">Certificates</span></a> </li>
                  </ul>
                  <!-- Tab panes -->
                ...
                <div class="col-md-7 mainContent middlecontent">
                  <div class="card-body p-b-0 bnrp0">
                      ...
                      <div class="tab-content">
                          <div class="tab-pane p-20 active" id="{{route('adoptions.certificates')}}" role="tabpanel">
                      ...
                      <div class="row">
                          ...
                          <div class="col-md-12 col-sm-12 col-lg-12">
                                ...
                                <div class="row">
                                  <div class="col-md-12">
                                    <div class=" borderline  d-flex justify-content-between">
                                      @php
                                      $getFamilyRoleUserInfo = FamilyRole::find($certificate->trial_family_role);
                                      $getFamilyRoleMatcherInfo = FamilyRole::find($certificate->adoptee_family_role);
                                      @endphp

                                    <div class="userA">
                                    @if($certificate->matcher_id)
                                      <img src="{{ url('uploads')}}/{{$certificate->matcherid->profile_pic}}" class="img-responsive"/>
                                    @endif
                                    <div class="usenamwe">
                                      <h4>
                                        @if($certificate->matcher_id)
                                        <a href="{{ url('userprofile')}}/{{base64_encode($certificate->matcher_id)}}">{{$certificate->matcherid->display_name_on_pages}}</a>
                                        @endif
                                      </h4>
                                    </div>
                                  </div>
                                  ...
                                  @if($certificate->user_id)
                                    <img src="{{ url('uploads')}}/{{$certificate->userid->profile_pic}}" class="img-responsive"/>
                                  @endif
                                  <div class="usenamwe">
                                    <h4>
                                      @if($certificate->user_id)
                                      <a href="{{ url('userprofile')}}/{{base64_encode($certificate->user_id)}}">{{$certificate->userid->display_name_on_pages}}</a>
                                      @endif
                                    </h4>
                                  </div>
                                </div>
                            ...
                            <div class="request-body">
                              @if($certificate->adopt_is_accepted == 1 && $certificate->matcher_id == Auth::user()->id)
                                <p class="remaining_time"><b>Your</b> & <b>{{$certificate->userid->display_name_on_pages}}</b> adoption certificate.</p>
                              @endif
                              @if($certificate->adopt_is_accepted == 1 && $certificate->user_id == Auth::user()->id)
                                <p class="remaining_time"><b>Your</b> & <b>{{$certificate->matcherid->display_name_on_pages}}</b> adoption certificate.</p>
                              @endif
                            </div>
                            <div class="request-footer reqftr">
                              @if($certificate->adoption_success == 1 && $certificate->user_id == Auth::user()->id)
                                <span class="label label-success text-center">Sent</span>
                              @endif

                              @if($certificate->adoption_success == 1 && $certificate->matcher_id == Auth::user()->id)
                                <span class="label label-success text-center">Recieved</span>
                              @endif

                              @if($certificate->adopt_is_decline == 2)
                                <span class="label label-danger text-center">Declined</span>
                              @endif

                              @if($certificate->adopt_is_accepted == 1)
                                <span class="label label-success text-center">Adopted</span>
                              @endif

                              @if($certificate->adoption_success == 1 && $certificate->adopt_is_accepted == 1)
                                <a href="{{ url('certificate')}}/{{base64_encode($certificate->id)}}" class="btn btn-success">See Certificate</a>
                              @endif
